...
for {set i 0} {$i <  $val(total)} {incr i} {
	...
	if {$j == 0} {
		...
		$node_($i) set X_ [expr 60.0 + $j]
		$node_($i) set Y_ $k
	} elseif {$j < 4} {
		$node_($i) set X_ [expr 2*60.0 + $j ]
		$node_($i) set Y_ $k
	} elseif {$j < 8} {
		$node_($i) set X_ [expr 3*60.0 + $j - 4]
		$node_($i) set Y_ $k
	} elseif {$j < 16} {
		$node_($i) set X_ [expr 4*60.0 + $j - 8]
		$node_($i) set Y_ $k
	}
    $god_ setasParentID [expr ($i - ($i%16) + ($i%16)/2) *100 + $i]
     
    if {[expr $i] >= $val(firstLeafNode)} {
