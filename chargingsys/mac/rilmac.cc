...
void RILMac::recv(Packet *p, Handler *h) {
  ...
  printf("node %d \tRILMac::recv:- msg in direction %d\tnext_hop %d\n",index_,hdr->direction(),hdr->next_hop());
...
void RILMac::recvHandler() {
  ...
  printf("RILMac::recvHandler: node:%d\n",index_);
...
void RILMac::sendHandler() {
  double boundtime = 0.01;
  printf("RILMac::sendHandler: node:%d\n",index_);
